...
Install only what you need. Every additional extension adds attack surface.
...
A Chrome extension with millions of users suddenly received an update with new code. The original developer sold the extension to a company that injected tracking scripts. Users received no notification about the ownership change. The new script collected browsing data and sent it to the buyer's servers. Thousands of users were affected before the security community discovered the code changes. This shows that even "trusted" extensions can become threats when ownership changes hands.
